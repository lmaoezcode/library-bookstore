...
CREATE TABLE books (
    ...
    promo_price REAL DEFAULT 0 CHECK(promo_price>=0),
    rent_price REAL DEFAULT 0 CHECK(rent_price >= 0), -- Giá cho thuê (nếu có)
    promo_rent_price REAL DEFAULT 0 CHECK(promo_rent_price>=0),
...
CREATE TABLE orders (
    ...
    status TEXT CHECK(status IN ('pending','processing', 'ready_to_ship','shipping', 'delivered', 'cancelled')) DEFAULT 'pending', -- Trạng thái đơn
...
CREATE TABLE borrows (
    ...
    status TEXT CHECK(status IN ('pending','approved','ready_to_pick','borrowing', 'returned', 'overdue','canceled')) DEFAULT 'pending', -- Đang mượn | Đã trả | Quá hạn
...
CREATE TABLE borrow_items (
    ...
    status TEXT CHECK(status IN
        ('pending','approved','ready_to_pick','borrowing','returned','overdue','canceled')
    ) DEFAULT 'pending',
